...
//const corsN = require ('cors');


var mysqll = require('mysql');  

var dbConnetion = mysqll.createConnection({  
...
dbConnetion.connect(function(err) {   
  if (!!err){
...
const db = require('./_helpers/db');//malith's db
...
app.use(bodyParser.urlencoded({ extended: false }));        //dup
app.use(cookieParser());
app.use(cors({ origin: (origin, callback) => callback(null, true), credentials: true }));  //dup
// api routes
app.use('/accounts', require('./accounts/accounts.controller'));
app.use('/billing', require('./billing/billings.controller'));
app.use('/services', require('./services/services.controller'));
app.use('/servicestationservices', require('./servicestationservices/servicestationservices.controller'));
app.use('/vehicles', require('./vehicle/vehicles.controller'));
app.use('/appointments', require('./appointment/appointments.contoller'));
app.use('/vehicleowner', require('./vehicleowner/vehicleowners.controller'));
app.use('/servicestation', require('./servicestation/servicestation.controller'));
app.use('/appointservice', require('./appointservices/appointservices.controller'));
app.use('/inventory', require('./inventory/inventory.controller'));
app.use('/requestedstation', require('./requestedstations/requestedstation.controller'));
app.use('/requestatserv', require('./reqstationservices/reqstationservice.controller'));
app.use('/bays', require('./bays/bays.controller'));
...
async function sendEmail_namu({ to, subject, html, from = config.emailFrom }) {
...
app.get('/get_user_id',function(req,res){
    ...
    var sql = 'Select * from vehicle_owner Where email = ?';

    dbConnetion.query(sql,req.query.email,function(err,rows){
        if(err) throw err;
        res.send(rows);
    });
...
app.get('/get_station_id',function(req,res){

    var sql = 'Select * from service_station Where email = ?';
...
app.post('/send_accpeted_declined_mail',

async function sendVerificationAcceptedEmail(req,res) {

    console.log("accpt/declined email func called");

    emailAttrs = req.body;

    const account = await db.Account.findOne({ where: { email: emailAttrs[0] } });
    ...
    await sendEmail_namu({
        to: account.email, //// methanta
        subject: 'New Sign-Up - AutoDoc',
        html: `<h4>AutoDoc Sign up verficication process</h4>
               ${message}`
    });
    res.status(200).send({"messagee":" accepted/declined email sent :-)"});
}


)

app.delete('/delete_station',function(req,res){

    var sql = "DELETE FROM service_station WHERE st_id = ? ";
    dbConnetion.query(sql,req.query.st_id,function(err,rows){
        if(err) throw err;
    });
    res.status(200).send({"messagee":"   deleted station successful "}); 

});

app.delete('/delete_owner',function(req,res){

    var sql = "DELETE FROM vehicle_owner WHERE owner_id = ? ";
    dbConnetion.query(sql,req.query.owner_id,function(err,rows){
        if(err) throw err;
    });
    res.status(200).send({"messagee":"   deleted vehicle owner successful "}); 
...
app.get('/inv', function (req, res) {
    dbConnetion.query('SELECT * FROM inventory_items WHERE st_id=?',req.query.ID, function (error, rows) {
...
app.get('/search', function (req, res) {
    ...
    dbConnetion.query('SELECT * FROM inventory_items WHERE item_type=? AND st_id=?',[req.query.ID1,req.query.STID], function (error, rows) {
        if (error) throw error;
        return res.send(rows);
    });
});
...
app.post('/addinv',function(req,res){
    console.log(req.body);
     let values = [req.body[0],req.body[1],req.body[2],req.body[3] ,req.body[4] ];
             

     var sql = 'INSERT INTO inventory_items SET ?';
     var sql2 =`INSERT INTO inventory_items (item_type,item_name,quantity,price,st_id) VALUES (?,?,?,?,?)`;
     dbConnetion.query(sql2 , values, function(err, result) {
        if (err) throw err;
        res.status(200).send({"messagee":" data received :-)"});  
    ...
    app.delete('/deleteinv',function(req,res){
        console.log(req.body);
        console.log("server works");
                 
         var sql = 'DELETE FROM inventory_items WHERE item_id=? ';
         
         dbConnetion.query(sql , req.query.item_id, function(err, rows) {
            if (err) throw err;
            res.status(200).send({"messagee":" data deleted :-)"});  
    ...
    app.put('/updateinv',function(req,res) {
        ...
        let values = [req.body[1],req.body[2],req.body[3] ,req.body[4],req.body[5] ,req.body[0]];
                 
         var sql = 'INSERT INTO inventory_items SET ?';
         var sql2 ='UPDATE inventory_items SET item_type=?,item_name=?,quantity=?,price=?,st_id=? WHERE item_id=?';
         dbConnetion.query(sql2 , [req.body[1],req.body[2],req.body[3] ,req.body[4],req.body[5] ,req.body[0]], function(err, rows) {
            if (err) throw err;
            res.status(200).send({"messagee":" data received :-)"});  
    ...
    app.get('/inv1', function (req, res) {
     
        dbConnetion.query('SELECT * FROM inventory_items WHERE st_id =? and item_id=?',[req.query.stationId,req.query.InventoryId], function (error, rows) {
            if (error) throw error;
            return res.send(rows);
        });
    });
...
app.get('/users', function (req, res) {
    ...
    dbConnetion.query('SELECT * FROM employee where st_id =?',req.query.stationId, function (err, rows) {
        if (err) throw err;
        return res.send(rows);
    });
});

app.get('/bays', function (req, res) {
     console.log("get base eke");
    dbConnetion.query('SELECT * FROM bay where st_id =?',req.query.stationId, function (err, rows) {
        if (err) throw err;
        return res.send(rows);
    });
});
...
app.get('/user1', function (req, res) {
    ...
    dbConnetion.query('SELECT * FROM employee where st_id =? and emp_id=?',[req.query.stationId,req.query.employeeId], function (err, rows) {
        if (err) throw err;
        return res.send(rows);
    });
});
...
app.post('/adduser',function(req,res){
    console.log(req.body);
     let values = [req.body[0],req.body[1],req.body[2],req.body[3] ,req.body[4],req.body[5],req.body[6]];
             

     var sql = 'INSERT INTO employee SET ?';
     var sql2 =`INSERT INTO employee (emp_name,emp_age,salary,mobile,role,bay_id,st_id) VALUES (?,?,?,?,?,?,?)`;
     dbConnetion.query(sql2 , values, function(err, rows) {
        if (err) throw err;
        res.status(200).send({"messagee":" data received :-)"});  
...
app.put('/updateuser',function(req,res) {
    ...
    let values = [req.body[1],req.body[2],req.body[3] ,req.body[4],req.body[5],req.body[6],req.body[7],req.body[8] ,req.body[0]];
             
     var sql = 'INSERT INTO employee SET ?';
     var sql2 ='UPDATE employee SET emp_name=?,emp_age=?,salary=?,mobile=?,role=?,status=?,bay_id=?,st_id=? WHERE emp_id=?';
     dbConnetion.query(sql2 , [req.body[1],req.body[2],req.body[3] ,req.body[4],req.body[5],req.body[6],req.body[7],req.body[8] ,req.body[0]], function(err, rows) {
        if (err) throw err;
        res.status(200).send({"messagee":" data received :-)"});  
...
app.delete('/deleteemp',function(req,res){
    console.log(req.body);
    console.log("server awa");
             
     var sql = 'DELETE FROM employee WHERE emp_id=? ';
     //var sql2 =`INSERT INTO employee (emp_id,emp_name,emp_age,salary,mobile,role,bay_id,st_id) VALUES (?,?,?,?,?,?,?,?)`;
     dbConnetion.query(sql , req.query.emp_id, function(err, rows) {
        if (err) throw err;
        res.status(200).send({"messagee":" data deleted :-)"});  
