...
export class AdminpanelComponent implements OnInit {
  ...
  view_regPDF(reg_doc_name:string){
    let url = "assets/img/"+reg_doc_name+".pdf";
    window.open(url);
  }

  view_regPDF_of_ValidStations(st_id:string){
    let url = "assets/img/REG-"+st_id+".pdf";
    window.open(url);
  }
  ...
  delete_station(AllServiceStationsRow:any){
    
    if( confirm("\n\nAre you sure you want to delete "+AllServiceStationsRow.st_name+" - "+AllServiceStationsRow.town+" from the database? ") ){
      this._adminServiceVariable.delete_station(AllServiceStationsRow.st_id)
      .subscribe(
        data=> {console.log('successfully deleted service Station!!!',data);       
          this.ngOnInit();  },
        error=>console.log('delete station error!!',error)
        
        );
       }    
  }

  delete_owner(VehicleOwnerRow:any){
    if( confirm("\n\nAre you sure you want to delete "+VehicleOwnerRow.first_name+" "+VehicleOwnerRow.last_name+" from the database?\nOwner ID: "+VehicleOwnerRow.owner_id) ){
      this._adminServiceVariable.delete_owner(VehicleOwnerRow.owner_id)
      .subscribe(
        data=> {console.log('successfully deleted vehicle owner!!!',data);       
          this.ngOnInit();  },
        error=>console.log('delete vehicle owner error!!',error)
...
export class Servicerequestview {
  ...
  change_account_table(email:string, isDecline:string){ //accepted, declined
    ...
    .subscribe(
      data=> {console.log('successfully accepted or declined account!!!',data);  
                this.send_accpeted_declined_mail(email,isDecline);
      },
  ...
  send_accpeted_declined_mail(email:string,isDecline:string){     //accepted, declined
   
    console.log("declined/accept  ts function called");
    
    let account_email_accpt_arr =[email,isDecline];
    this._adminServiceVariable.send_accpeted_declined_mail(account_email_accpt_arr)
    .subscribe(
      data=> {console.log('successfully sent accepted or declined account EMAIL !!!',data); 
      console.log(isDecline,"isdeclined ???");
      if(isDecline=='declined'){
        window.location.reload();
      }                              },
      error=>console.log('account accept/decline EMAIL error!!',error) 
      );
